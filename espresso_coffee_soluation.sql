...
-- import table through Table data import wizard 
...
GROUP BY c.city_name,cc.customer_id,cc.customer_name
  ...
  SELECT city_name, customer_id,customer_name,total_revenue,rnk
  FROM top_customers
  WHERE rnk<=3;
...
SELECT c.city_name, COUNT(DISTINCT cc.customer_id) as unique_cus_count, COUNT(s.sale_id) AS total_product_ordered
FROM city AS c 
JOIN customers AS cc ON c.city_id = cc.city_id
...
write a query to find the identify top 3 city based on highest sales, return city name, total sale, total rent, total customers, 
averge revenue, average rent, estimated coffee consumer */
